...
def test_spectral_model(like, pbh_ra, pbh_dec):
    ...
    npred1000 = like._srcCnts('PBH_Source')[3:]
...
def simulate_and_analyze(temp, dist, gamma_fraction, v_disk, v_dm, z, j):
    ...
    ts, spectral_match, chi2value, npred, npred1000 = test_spectral_model(like,pbh_ra, pbh_dec)
    print "TS: " + str(ts)
    ...
    if source_detected:
        ...
        like = perform_likelihood(1000, 100000, 3, pbh_ra, pbh_dec, prefix, pbh_present=False)

        my_arr, ps_arr = make_model_cube(prefix,like)
    ...
    if z == 0:
        ...
        pickle.dump({"JOB_ID":os.environ['LSB_JOBID'],"Temperature":temp, "Distance":dist,  "Source_Detected":source_detected, "GLON":SkyCoord(ra=pbh_ra*u.degree, dec=pbh_dec*u.degree,frame='icrs').galactic.l.degree, "GLAT":SkyCoord(ra=pbh_ra*u.degree, dec=pbh_dec*u.degree,frame='icrs').galactic.b.degree, "V_Recovered":v_recovered , "Spectral_Match": spectral_match, "MIN_CHI2":chi2value, "V_True": v_tang, "TS":ts, "Sig": significance}, file)
        file.close()
...
v_disk = np.array([300.0, 270.0, 100.0],'d')
v_dm = np.array([350.0, 250.0, 150.0],'d')
z = int(sys.argv[4])
j = int(sys.argv[5])
